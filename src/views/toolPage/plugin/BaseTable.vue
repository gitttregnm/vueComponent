...
import BaseTable from "@/components/plugin/BaseTable";
export default {
  name: "BaseFormDesc",
  data() {
    return {
      ...
      tableData1: [
        ...
        },
        {
          ...
          explain:
            "用来设置table初始化时哪些checkBox可点击（对有复选框的table内容生效）可设置多个，多个可并列做为筛选条件。",
          type: "array",
          optionalValue:
            "例如： [{key: 'color',val: '蓝色'}] 意思为：当前行 color字段的值 === 蓝色 的checkbox 设置为不可点击 ",
          defaultValue: "[]"
        },
        {
          parameter: "summary",
      ...
      tableData2: [
        ...
        },
        {
          parameter: "sumText",
          explain:
            "合计行第一列的文本。注意：此字段会再设置了 汇总方法summaryMethod后失效 ",
          type: "String",
          optionalValue: "",
          defaultValue: ""
        },
      ...
      tableData3: [
        ...
        },
        {
          parameter: "formatter",
          explain:
            "表格的单元格格式化展示函数。如果isHtml = true 则 此函数可以 return ’html 格式内容‘",
          type: "Function",
      ...
      tableData4: [
        ...
        },
        {
          ...
          explain:
            "自定义数据过滤方法。函数值返回值必须为 true或false。满足条件的数据返回 true 就会显示。",
          type: "function",
      ...
      tableData5: [
        ...
        },
        {
          ...
          explain:
            "开启了checkbox一列显示后，如果选择了表头的checkbox 则会触发此函数。",
          parameter: "all",
  ...
  mounted() {
    ...
    import BaseTable from '@/component/plugin/BaseTable'
    export default {
      data() {
        return {
          defaultSort: {prop: 'id', order: 'descending'}, // 设置默认排序规则
  ...
  methods: {
    ...
    getCellCilck(row, column, cell, event) {
      console.log(
        `获取当前单元格相关的信息 行值：${row}  列值：${column} 单元格值：${cell} 事件：${event}`
      );
    },
    getCurrentChange(currRow) {
      console.log(`当前变化后行信息 ${currRow}`);
